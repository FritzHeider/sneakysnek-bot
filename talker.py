...
def voice():
    ...
    resp = VoiceResponse()
    resp.say(f",,,Hello {open('Details/Client_Name.txt', 'r').read()}, WE ARE CALLING ABOUT SOME SUSPICIOIS ACTIVITY ON your {open('Details/Company Name.txt', 'r').read()} account, your {open('Details/Company Name.txt', 'r').read()} account password is CURRENTLY trying to be reset from unknow location,")
...
def gatherotp():
    ...
    if 'Digits' in request.values:
        ...
        resp.say(f'Great, we have blocked the request. REMEMBER A {open("Details/Company Name.txt", "r").read()} EMPLOYEE WILL NEVER ASK YOU FOR YOUR PASSWORD,IF PROBLEMS PERSIST A {open("Details/Company Name.txt", "r").read()} AGENT WILL CALL YOU BACK TO FOLLOW UP,')
